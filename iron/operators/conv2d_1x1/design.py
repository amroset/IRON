...
def my_matmul(
    ...
    for col in range(n_aie_cols):
        ...
        if use_scalar:
            # Plain tile for the scalar kernel: [n, k] row-major when b_col_maj
            # (b[ic + oc*k]) else [k, n] row-major (b[ic*n + oc]).
            dims_to_stream = [(n, k), (k, 1)] if b_col_maj else [(k, n), (n, 1)]
        elif b_col_maj:
        ...
        # Output C
        if use_scalar:
            # Plain tile written by the scalar kernel: [n, m] row-major when
            # c_col_maj (c[p + oc*m]) else [m, n] row-major (c[p*n + oc]).
            dims_to_stream = [(n, m), (m, 1)] if c_col_maj else [(m, n), (n, 1)]
        elif c_col_maj:
