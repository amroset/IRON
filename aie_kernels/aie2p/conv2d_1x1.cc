...
// Scalar matmul == pointwise (1x1) conv over channels (rowA=pixels, colA=C_in,
// colB=C_out). This is the conv operator's scalar oracle: it consumes PLAIN
// row-major L1 tiles (design.py streams them plainly when use_scalar=True),
// which is why it can use direct [row, col] indexing.
//
// Accumulate in f32 (cast the bf16 inputs up before multiplying) so the products
// are not rounded to bf16 per step. With floor rounding, per-step bf16 products
